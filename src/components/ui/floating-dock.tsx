...
const FloatingDockMobile = ({ items, className }: { items: { title: string; icon: React.ReactNode; href: string }[]; className?: string }) => {
  ...
  return (
    <div className={cn('relative block md:hidden', className)}>
        ...
        {open && (
          <motion.div layoutId="nav" className="absolute bottom-full mb-2 inset-x-0 flex flex-col gap-2">
            {items.map((item, idx) => (
              ...
              >
                <Link href={item.href} key={item.title} className="h-10 w-10 rounded-full bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
      ...
      <button onClick={() => setOpen(!open)} className="h-10 w-10 rounded-full bg-gray-50 dark:bg-gray-800 flex items-center justify-center">
...
const FloatingDockDesktop = ({ items, className }: { items: { title: string; icon: React.ReactNode; href: string }[]; className?: string }) => {
  const mouseX = useMotionValue(Infinity) // let to const
  return (
    <motion.div
      onMouseMove={(e) => mouseX.set(e.pageX)}
      onMouseLeave={() => mouseX.set(Infinity)}
      className={cn('mx-auto hidden md:flex h-16 gap-4 items-end  rounded-2xl bg-gray-50 dark:bg-gray-700 px-4 pb-3', className)}
    >
      {items.map((item) => (
  ...
  return (
    <Link href={href}>
      <motion.div
        ...
        className="aspect-square rounded-full bg-gray-200 border-[1.5px] dark:border-none border-gray-50 dark:bg-gray-800 flex items-center justify-center relative"
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
